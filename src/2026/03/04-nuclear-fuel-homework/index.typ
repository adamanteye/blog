...
#let hw = [
  ...
  故至少需要 $m = 4.26 times 106 approx qty("452", "g")$
  ...
  因此膜孔的最大等效孔径应满足 $L < kl / 10$

  其中分子的平均自由程为 $kl = 1 / (sqrt(2) pi d^2 n)$

  对理想气体，有 $n = P / (k_B T)$

  故可得 $kl = k_B T / (sqrt(2) pi d^2 P)$
  ...
  即 $kl_(chem("U")chem("F")_6) approx qty("0.23", "um")$
  ...
  即 $kl_(chem("H")_2) approx qty("0.832", "um")$
  ...
  取工作气体为 $chem("U")chem("F")_6$, 其摩尔质量为 $M = qty("0.352", "kg/mol")$

  角速度为 $ko = v / a = 600 / 0.06 = qty("1.0e4", "rad/s")$

  气体在径向平衡时满足 $div(p/r) = rho ko^2 r$

  又因为理想气体有 $rho=(M p)/(R T)$

  所以 $dd(p)/p=(M ko^2 / (R T))r dd(r)$

  由此积分得压强分布 $p(r) = p(a) exp[-M ko^2 (a^2 - r^2) / (2 R T)]$

  离心机内部总滞留量按物质的量计算为 $n = int_V p / (R T) dd(V)$
  ...
  因而 $exp(-25.4) << 1$, 可近似取 $n approx (2 pi h p(a)) / (M ko^2)$
  ...
  $F = 1000 times 238 / 352 approx qty("676.2", "t/a")$
  ...
  $
    P = 676.2 times (0.00711 - 0.002) / (0.05 - 0.002)
    approx qty("71.99", "t/a")\
    W = 676.2 - 71.99 = qty("604.21", "t/a")
  $

  分离功采用价值函数 $V(x) = (1 - 2x) ln((1 - x)/x)$

  并有 $S = P V(x_p) + W V(x_w) - F V(x_f)$
  ...
  $
    S & approx 71.99 times 2.650 + 604.21 times 6.188 - 676.2 times 4.869 \
      & approx 637.13
  $
