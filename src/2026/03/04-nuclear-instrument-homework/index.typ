...
#let hw = [
  ...
  题设电阻热噪声输入功率谱密度为 $s_i(ko)=2 k T R / pi$, 因而输出噪声功率谱密度:

  $
    s_o(ko) = abs(H(j ko))^2 s_i(ko)
    = (2 k T R) / (pi (1 + ko^2 R^2 C^2))
  $
  ...
  $
    ol(v_o^2)
    = int_0^inft s_o(ko) dif ko
    = (2 k T R) / pi int_0^inft (dif ko) / (1 + ko^2 R^2 C^2)
    = (k T) / C
  $
  ...
  #table(
    ...
    [(1)], [输入电压噪声经反馈直接反映到输出], [$d ol(v_o^2)=d ol(v_n^2)$],

    [(2)], [输出端串联噪声在测量端直接出现], [$d ol(v_o^2)=d ol(v_n^2)$],

    [(3)],
    [输入电压噪声经噪声增益放大],
    [$d ol(v_o^2)=(1+C_i/C_f)^2 d ol(v_n^2)$],

    [(4)], [输出噪声源位于反馈环内, 理想负反馈将其抵消], [$d ol(v_o^2)=0$],

    [(5)], [输入电流噪声流过反馈电容], [$d ol(v_o^2)=d ol(i_n^2)/(ko^2 C_f^2)$],

    [(6)],
    [输入端并联 $C_i$ 处于虚地, 不改变电流噪声传递],
    [$d ol(v_o^2)=d ol(i_n^2)/(ko^2 C_f^2)$],

    [(7)],
    [前放后接 CR 高通, $H(j ko)=j ko R C/(1+j ko R C)$],
    [$d ol(v_o^2)=d ol(i_n^2) R^2 C^2/(C_f^2(1+ko^2 R^2 C^2))$],
  )

  其中 (3) 的结果说明, 串联输入电压噪声会随总输入电容增大而放大为更大的输出噪声.

  == 带宽为 $qty("1", "MHz")$ 时的等效噪声电荷

  输入电压噪声密度取 $e_n=qty("1", "nV/sqrt(Hz)")$, 等效噪声带宽
  $B=qty("1", "MHz")$. 题图中的滤波器在带宽内 $abs(H)^2=1$,
  因而输入电压噪声均方根值为:

  $
    v_("n,rms") = e_n sqrt(B)
    = qty("1", "nV/sqrt(Hz)") sqrt(qty("1e6", "Hz"))
    = qty("1", "uV")
  $
  ...
  $
    A_n = 1 + C_"in" / C_f = 1 + 9 / 1 = 10
  $

  输出噪声均方根值为 $qty("10", "uV")$. 用信号关系 $v_o=Q/C_f$ 折算到输入端, 有:

  $
    Q_"ENC" = C_f A_n v_("n,rms")
    = (C_f + C_"in") v_("n,rms")
    = qty("10", "pF") times qty("1", "uV")
    = qty("1.0e-17", "C")
  $
  ...
  $
    "ENC" = Q_"ENC" / e
    approx (1.0 times 10^(-17)) / (1.602 times 10^(-19))
    approx 62 e
  $
  ...
  + $a$ 噪声: 串联白电压噪声经输入总电容转换成电流噪声, 所以
    $d ol(i_a^2)=a_i^2 ko^2 dif ko$.
  + $b$ 噪声: 平行白电流噪声, 主要来自探测器漏电流散粒噪声和反馈电阻热噪声, 所以
    $d ol(i_b^2)=b_i^2 dif ko$.
  + $c$ 噪声: $1/f$ 电压噪声经输入电容转换后的项, 所以
    $d ol(i_c^2)=c_i^2 ko dif ko$.

  若 $A$ 为理想放大器且忽略 $R_f$, 输入电流噪声到输出的传递阻抗为
  $Z_f=1/(j ko C_f)$, 所以:

  $
    d ol(v_o^2)
    = abs(Z_f)^2 (
      d ol(i_a^2) + d ol(i_b^2) + d ol(i_c^2)
    )
  $
  ...
  $
    A(s) = A_0 / (1 + s tau_0), quad A_0 >> 1
  $

  令反馈导纳 $Y_f=1/R_f+s C_f$, 输入电流噪声到输出的精确传递阻抗为:
  ...
  $
    Z_t(s)
    approx 1 / (1/R_f + s C_f + s C_i / A(s))
    = 1 / (1/R_f + s(C_f + C_i/A_0) + s^2 C_i tau_0 / A_0)
  $
  ...
  $
    d ol(v_o^2)
    = abs(Z_t(j ko))^2
    (a_i^2 ko^2 + b_i^2 + c_i^2 ko) dif ko
  $

  反馈电阻使低频处 $Z_t$ 不再按 $1/ko$ 发散; 有限增益带宽又在高频端引入额外滚降.

  = 第十一次作业

  == CR-(RC) 成形电路的 ENC

  对等时间常数的一阶 CR-(RC) 成形, 输入电荷信号的峰值归一化后,
  可采用常用形状因子:
  ...
  #table(
    columns: 5,
    [$tau$], [串联项], [平行项], [$1/f$ 项], [总 ENC],

    [$qty("0.2", "us")$], [$173 e$], [$34.5 e$], [$120 e$], [$213 e$],

    [$qty("5", "us")$], [$34.5 e$], [$173 e$], [$120 e$], [$213 e$],
  )
  ...
  此时串联项和平行项各约 $77.2 e$, 加上 $1/f$ 项后:

  $
    "ENC"_"min" approx 162 e
  $

  == 不同成形时间下的信噪比

  噪声转角时间 $tau_c=qty("1", "us")$ 表示串联噪声和平行噪声在该成形时间相等.
  忽略与成形时间无关的 $1/f$ 项时:

  $
    "ENC"^2(tau) prop tau / tau_c + tau_c / tau
  $

  题给 $tau=tau_c$ 时 $eta_"opt"=1000$, 因而:

  $
    eta(tau)
    = eta_"opt" sqrt(2 / (tau/tau_c + tau_c/tau))
  $
  ...
  信噪比关于 $tau=tau_c$ 近似对称; 时间常数太小串联噪声主导, 太大平行噪声主导.

  == 脉冲光反馈前放后的 CR-RC 成形电路
  ...
  一个可行设计是采用一级 CR 微分和一级 RC 积分, 二者时间常数均为
  $qty("2", "us")$, 再加电压增益 $37$ 的放大级. 例如取:

  $
    C_1=C_2=qty("1", "nF"), quad R_1=R_2=qty("2", "kO")
  $

  则:

  $
    R_1 C_1 = R_2 C_2 = qty("2", "us")
  $
  ...
  $
    v_o(t)=V_i (t/tau) e^(-t/tau) u(t)
  $

  因而达峰时间和峰值为:

  $
    t_M=tau, quad V_"OM"=V_i/e
  $

  所以 $tau=qty("1", "us")$, $qty("2", "us")$, $qty("4", "us")$,
  $qty("8", "us")$ 时, 峰值不变, 波形只按时间轴等比例拉宽, 达峰时间分别为
  $qty("1", "us")$, $qty("2", "us")$, $qty("4", "us")$, $qty("8", "us")$.

  以峰值的 $1%$ 作为脉冲宽度边界, 解 $(t/tau)e^(1-t/tau)=0.01$ 得两个交点约为
  $0.0037 tau$ 和 $7.64 tau$, 因而:

  $
    t_W(1%) approx 7.64 tau
  $

  也就是说 $tau$ 增大几倍, 达峰时间和脉冲宽度也相应增大几倍.

  == 弹道亏损

  弹道亏损来自探测器电流脉冲宽度不可忽略时,
  成形电路不能把全部电荷按理想冲激输入那样 转化为同一峰值.
  如果成形时间与电荷收集时间同量级或更短, 不同收集时间的事件会得到不同 峰值,
  峰位被拉低并展宽, 因而会恶化能量分辨率.

  若探测器输出电流脉冲都是宽度相同的矩形, 则弹道亏损只造成共同的幅度降低,
  可通过能量刻度修正. 在这种理想情况下它不会额外展宽能峰,
  因而不会使能量分辨率变坏.
  真正造成分辨率恶化的是事件间收集时间或脉冲形状的差异.

  == 成形时间的折中

  对 CR-RC 能谱系统, 成形时间 $tau$ 的选择需要在三类效应之间折中:

  - 为了信噪比最好, 应取 $tau$ 接近噪声转角时间 $tau_c$,
    使串联噪声和平行噪声大致平衡.
  - 为了减小弹道亏损, 应增大 $tau$, 使成形时间明显长于探测器电荷收集时间,
    输出峰值对电流脉冲宽度不敏感.
  - 为了减小信号堆积, 应减小 $tau$, 使脉冲宽度变窄,
    高计数率下相邻事件更容易分开.

  因此实际取值不是单纯越大或越小越好, 而是选在噪声最优附近,
  并根据探测器收集时间和计数率作偏移: 低计数率、慢收集时可适当取大,
  高计数率时需要适当取小.

  == 阶跃输入下两级成形电路波形

  第一极为反相 CR 高通, 参数 $C_1=qty("1000", "pF")$, $R_1=qty("1", "kO")$,
  $R_2=qty("10", "kO")$, 故 $tau_1=R_1 C_1=qty("1", "us")$. 对
  $V_"iM"=qty("0.1", "V")$ 阶跃输入:

  $
    v_2(t) = - (R_2/R_1) V_"iM" e^(-t/tau_1)
    = -qty("1.0", "V") e^(-t/qty("1", "us"))
  $

  第二极为反相 RC 低通, $R_3=qty("1", "kO")$, $R_4=qty("10", "kO")$,
  $C_4=qty("100", "pF")$, 所以 $tau_2=R_4 C_4=qty("1", "us")$. 两级时间常数相同,
  总传递函数为:

  $
    V_o(s)/V_i(s)=100 s tau/(1+s tau)^2, quad tau=qty("1", "us")
  $

  因此:

  $
    v_o(t)=100 V_"iM" (t/tau) e^(-t/tau)
    = qty("10", "V") (t/tau) e^(-t/tau)
  $

  输出达峰时间 $t_M=tau=qty("1", "us")$, 峰值
  $V_"OM"=qty("10", "V")/e approx qty("3.68", "V")$.

  == 不同 $R_f$ 和 $R_1$ 下的 $v_1(t)$ 与 $v_o(t)$

  题图中 $C_f=qty("2", "pF")$, $C_1=qty("2000", "pF")$, $R_2=qty("1", "kO")$. 记
  $V_Q=Q/C_f$. 前放输出为:

  $
    v_1(t)=cases(
      -V_Q "if" R_f -> inft,
      -V_Q e^(-t/tau_f) "if" R_f < inft,
    )
  $

  其中 $tau_f=R_f C_f$. 当 $R_f=qty("1e8", "ohm")$ 时, $tau_f=qty("200", "us")$.

  对后级耦合网络, 若 $R_1 -> inft$, 则为 CR 高通, 时间常数
  $tau_h=R_2 C_1=qty("2", "us")$. 若 $R_1$ 有限, 低频分压系数 $K=R_2/(R_1+R_2)$,
  快时间常数 $tau_h=(R_1 parallel R_2)C_1$.

  四种情况为:

  + $R_f -> inft$, $R_1 -> inft$: $v_1(t)=-V_Q$;
    $v_o(t)=-V_Q e^(-t/qty("2", "us"))$. 输出初值 $-V_Q$, 以 $qty("2", "us")$
    衰减到零.
  + $R_f=qty("1e8", "ohm")$, $R_1=qty("100", "kO")$:
    $v_1(t)=-V_Q e^(-t/qty("200", "us"))$. 由于
    $R_1 C_1=R_f C_f=qty("200", "us")$, 慢极点近似抵消,
    $tau_h=(100 " kO" parallel 1 " kO") times 2000 " pF" approx qty("1.98", "us")$,
    所以 $v_o(t) approx -V_Q e^(-t/qty("1.98", "us"))$.
  + $R_f=qty("1e8", "ohm")$, $R_1 -> inft$:
    $v_1(t)=-V_Q e^(-t/qty("200", "us"))$. 后级高通与前放衰减共同作用:
    $
      v_o(t)
      = -V_Q/(1-tau_h/tau_f)
      (e^(-t/tau_h) - (tau_h/tau_f)e^(-t/tau_f))
    $
    其中 $tau_h=qty("2", "us")$, $tau_f=qty("200", "us")$. 波形先从约 $-V_Q$
    快速衰减, 后面留下约 $1%$ 幅度的反向长尾.
  + $R_f -> inft$, $R_1=qty("4", "kO")$: $v_1(t)=-V_Q$. 此时 $K=1/(4+1)=0.2$,
    $tau_h=(4 " kO" parallel 1 " kO") times 2000 " pF"
    =qty("1.6", "us")$. 输出为:
    $
      v_o(t)=-V_Q (0.2 + 0.8 e^(-t/qty("1.6", "us")))
    $
    即初值仍为 $-V_Q$, 但不回到零, 而是以 $qty("1.6", "us")$ 衰减到 $-0.2 V_Q$
    的平台.
  = 第十二次作业
  ...
  SDD 的核心优势是把大面积入射窗口和小面积收集阳极分离.
  横向漂移场把电子云输运到很小的阳极, 所以阳极电容和前放输入电容远小于同面积硅
  PIN 探测器. 串联噪声近似随输入电容增大而增大, 因此 SDD 可获得更低 ENC. 同时
  SDD 漏电流较小, 漂移收集路径清楚,
  并且小电容允许较短成形时间下保持较好噪声性能. 对低能 X 射线,
  电子学噪声常是决定 FWHM 的主要项, 因而 SDD 的能量分辨率优于硅 PIN.

  == 不同 ENC 下 $qty("5.9", "keV")$ X 射线能量分辨率

  定义: $E=qty("5.9", "keV")=qty("5900", "eV")$, $w=qty("3.6", "eV")$, $F=0.12$.
  平均电子-空穴对数 $ol(N)=E/w approx num("1.64e3")$, 法诺涨落为
  ...
  #table(
    columns: 4,
    [ENC], [$ks_N$ / e], [FWHM], [相对分辨率],

    [$4$], [$14.6$], [$qty("124", "eV")$], [$2.10%$],

    [$10$], [$17.2$], [$qty("146", "eV")$], [$2.48%$],
  )
  ...
  即: 在效率和测量时间相同的前提下, NaI 的能量分辨率差导致能窗宽得多,
  本底计数按能窗宽度增加, 而 MDA 近似随 $sqrt(B)$ 增大. 因此 NaI
  的最低可探测活度约为 HPGe 的 $5.9$ 倍.

  == SiPM 的工作原理

  SiPM 由大量并联微单元组成, 每个微单元本质上是工作在盖革模式的雪崩光电二极管.
  偏置电压高于击穿电压, 即 $kD V=V_"bias"-V_"bd">0$.
  光子在耗尽区产生电子-空穴对后, 载流子触发自持雪崩, 单个微单元输出近似固定电荷
  $Q_"cell"=C_"cell" kD V$. 串联的淬灭电阻使雪崩电流降低并把结电压拉回击穿以下,
  随后微单元按 $R_q C_"cell"$ 时间常数恢复.

  多个微单元同时接收光子时, 输出电荷近似为 $Q_"out"=N_"fire" Q_"cell"$, 在线性区
  $N_"fire" prop N_kg "PDE"$, 所以输出脉冲幅度与入射光子数成正比.
  当入射光子数接近微单元总数时, 已触发单元不能立即再次响应, 输出出现饱和,
  可近似写成 $N_"fire"=N_"cell"(1-e^(-N_kg "PDE"/N_"cell"))$.
  实际性能还受暗计数, 光学串扰, 后脉冲和恢复时间限制.

  = 第十五次作业

  == 伽马相机的基本工作原理

  伽马相机用于测量体内或样品中放射性核素发出的 $kg$ 射线空间分布. 其核心是把每个
  $kg$ 光子的入射方向, 沉积能量和位置转换成一个图像事件,
  再由大量事件累计成二维活度分布.

  首先, 铅准直器只允许接近特定方向的 $kg$ 光子进入探测器,
  斜入射光子大多被准直器吸收.
  因而探测器晶体上的位置可以近似对应到被测物中的投影位置.
  准直器决定了伽马相机的主要空间分辨率和灵敏度: 孔径越小,
  分辨率越好但计数率越低.

  进入探测器的光子通常在大面积 NaI(Tl) 闪烁晶体中发生光电效应或康普顿散射,
  沉积能量转化为闪烁光. 晶体后面的光电倍增管阵列把闪烁光转换为电信号.
  各光电倍增管信号幅度 $A_i$ 反映光斑在晶体中的分布, 可用 Anger
  位置逻辑估计相互作用点:

  $
    x = (sum_i x_i A_i) / (sum_i A_i), quad
    y = (sum_i y_i A_i) / (sum_i A_i), quad
    E prop sum_i A_i
  $

  其中 $E$ 用于能量甄别. 脉冲高度分析器只接受落在目标光电峰能窗内的事件,
  从而抑制康普顿散射光子和环境本底. 被接受事件按 $(x,y)$ 坐标填入图像矩阵,
  ...
  + 增加测量时间: 本底主导时 MDA 约按 $1/sqrt(T)$ 降低.
  + 选择干扰更小且发射概率更高的特征 $kg$ 射线,
    并用本底扣除或反符合技术抑制非目标计数.
]
