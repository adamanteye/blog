...
#let hw = [
  ...
  由 $r'(0_+) = 2$ 得 $C_2 - C_1 = 2$

  故 $C_2 = 3$.

  因此系统的零输入响应为 $r_"zi"(t) = e^(-t)(cos t + 3 sin t) u(t)$

  == 2

  在零初始条件下取拉氏变换, 有 $s^2 R(s) + s R(s) + R(s) = s E(s) + E(s)$

  故系统函数为 $H(s) = R(s) / E(s) = (s + 1) / (s^2 + s + 1)$

  将分母配方: $s^2 + s + 1 = (s + 1/2)^2 + 3/4$
  ...
  其中 $f(t) = e^(-t) u(t) + 3 kd(t)$

  注意到 $int_(-inft)^(+inft) e(tau) f(t - tau) dd(tau) = (e * f)(t)$

  在零初始条件下取拉氏变换: $s R(s) + 5 R(s) = E(s) F(s) - E(s)$

  而 $F(s) = 1/(s + 1) + 3$

  故 $H(s) = R(s) / E(s) = (F(s) - 1) / (s + 5)$
  ...
  因此单位冲激响应为 $h(t) = (1/4 e^(-t) + 7/4 e^(-5 t)) u(t)$

  == 4

  === (a)

  由图可知 $f_2(t) = kd(t + 2) + kd(t - 2)$

  因此 $(f_1 * f_2)(t) = f_1(t + 2) + f_1(t - 2)$
  ...
  由图可写 $f_1(t) = 2 (u(t) - u(t - 1))$, $f_2(t) = sin t (u(t) - u(t - pi))$.
  ...
  + 当 $t < 0$ 或 $t > pi + 1$ 时, 两函数无重叠, $(f_1 * f_2)(t) = 0$
  ...
  所以漂移速度为 $v_e = mu_e E approx qty("2.0e4", "m/s")$,
  $v_i = mu_i E approx qty("20", "m/s")$.

  对阳极而言, 权电场为 $E_w = num("1") / d = qty("100", "m^-1")$
  ...
  所以 $i_e = (Q_0 v_e) / (d - d_1) approx qty("1.28e-7", "A")$,
  ...
  所以全耗尽电压约为 $V_"fd" approx qty("70", "V")$
  ...
  并且 $int_(num("0"))^d abs(E(x)) dd(x) = V_R$
  ...
  因此若从左侧背电极起算, 产生位置为 $x_0 = (num("3") D) / num("4")$

  对右侧阳极而言, 权电势为 $phi_w(x) = x / D$

  所以初始权电势 $phi_w(x_0) = num("3") / num("4")$
  ...
  故总输出电荷量为 $kD Q = n (kD Q_e + kD Q_h) = n e$
  ...
  设输入矩形电流脉冲为 $i_s(t) = I_0 (u(t) - u(t - t_w))$

  并联 $"RC"$ 电路满足 $C dv(v, t) + v/R = i_s(t)$

  因此在脉冲作用期间 $0 < t < t_w$, $v(t) = I_0 R (1 - e^(-t/(R C)))$
  ...
  + 当 $t_w$ 远大于 $R C$ 时, 输出先在约几个 $R C$ 内快速上升并趋近于稳态值
    $v approx I_0 R$ 在脉冲持续的大部分时间里近似保持平台.
    脉冲结束后再以时间常数 $R C$ 指数下降. 这时波形近似为圆角矩形脉冲.
  ...
  由外特性曲线可直接读出 3 个区间的电压增益, 即各段斜率 $K = dd(v_o) / dd(v_i)$
  ...
  反馈电容在输入端的密勒等效值为 $C_M = C_f (1 - K)$

  于是 $C_"M1" = C_"M3" = num("1") times (1 - (-200)) = qty("201", "pF")$,
  ...
  右图为同相放大器. 反馈系数 $kb = R_2 / (R_1 + R_2) = 1 / 100$

  且 $v_o = A (v_i - kb v_o)$

  所以闭环增益 $v_o / v_i = A / (1 + A kb)$
  ...
  输入端列 KCL: $i(t) = C_i dv(v_i, t) + C_f dv(v_i - v_o, t)$

  在拉氏域令 $i(t) = Q kd(t)$, 即 $I(s) = Q$, 可解得
  $V_o(s) = -Q (A - R C_f s) / (s (C_T + K s))$

  其中记 $C_T = C_i + (1 + A) C_f$, $K = R (C_i C + C_i C_f + C C_f)$.
  ...
  而稳态幅度 $lim_(t -> oo) v_o(t) approx - A Q / ((1 + A) C_f) approx -Q / C_f$
  ...
  输出不再长期保持平台, 而是在到达峰值后按较长时间常数 $tau_f approx R_f C_f$
  缓慢回到基线. 因此波形由"快上升 + 平顶"变为"快上升 +
  慢恢复"的典型电荷灵敏前放脉冲.

  == 6

  电荷灵敏前放的闭环上限频率近似为 $f_H approx "GBW" / (1 + C_i / C_f)$

  于是上升时间 $t_r approx 0.35 / f_H = 0.35 (1 + C_i / C_f) / "GBW"$
