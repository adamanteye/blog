...
#thm([$delta$函数的性质])[$
   delta(a x) & =1 / (|a|)delta(x) \
     delta(x) & =1 / (2pi)integral_(-infinity)^infinity e^(i k x)dd(k) \
   x delta(x) & =0 \
...
#proof[平均值不变由@ehrenfest 说明,概率分布的证明如下
  ...
  现在考虑概率的变化
  $ dv(, t) abs(a_k (t))^2=dv(a_k^*, t)a_k + a_k^* dv(a_k, t) $由于$   dv(
      a_k^*,
      t
...
#pst(
  "全同性假设",
)[全同粒子体系中,可观测量对任何两个粒子交换是不变的,即任意两个全同粒子的交换,都不改变体系的物理状态.] <identity-postulate>
#coll(
  "全同粒子体系波函数的性质",
)[根据@exchange-operator 以及@identity-postulate,全同粒子体系的波函数$psi$满足$ P_(i j) psi & =C psi \
            C & =plus.minus 1 $]
