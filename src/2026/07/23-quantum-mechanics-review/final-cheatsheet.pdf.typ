...
#def("角动量升降算符")[$
  ...
  $]
$
  &[Jp,Jm]=2hbar J_z, [J_z,Jpm]=plus.minus hbar Jpm\ &[J_x,Jpm]=minus.plus hbar J_z,[J_y,Jpm]=-hbar J_z\ &[va(J)^2,Jpm]=0
$$ Jpm ket(j m)=hbar sqrt(j(j+1)-m(m plus.minus 1)) ket(j m plus.minus 1) $
== 自旋
#def("Pauli矩阵")[$ sigma_x=mat(0, 1; 1, 0),sigma_y=mat(0, -i; i, 0),sigma_z=mat(1, 0; 0, -1) $$
    sigma_alpha sigma_beta=delta_(alpha beta)+i sum_gamma epsilon_(alpha beta gamma) sigma_gamma\ sigma_alpha=sigma_alpha^dagger
  $]
#def("磁矩算符")[$ va(mu)=g mu_B / hbar va(j)\ va(j)=va(s),va(l), g_l=-1,g_s=-2 $]
#def("磁矩磁场相互作用能")[$ W=-(va(mu_l)+va(mu_s))dprod va(B) $]
#def("旋量波函数")[$
    psi(va(r), s_z)=mat(psi_1(va(r),+hbar / 2); psi_2(va(r),-hbar / 2))
  $在哈密顿量不含自旋或可以表示成与自旋部分之和的时候$ psi(va(r), s_z)=psi(va(r))chi(s_z)\ alpha=chi_(+1\/2)(s_z)=mat(1; 0)\ beta=chi_(-1\/2)(s_z)=mat(0; 1) $]
== 耦合与非耦合表象的基底变换
$ ket(j_1 j_2 j m)=sum_(m_1=-j_1)^(j_1)sum_(m_2=-j_2)^(j_2) C_(j_1 m_1 j_2 m_2)^(j_1 j_2 j m) ket(j_1 m_1 j_2 m_2) $$
  C_(j_1 m_1 j_2 m_2)^(j m)=braket(j_1 m_1 j_2 m_2, j_1 j_2 j m)
$
== 电子耦合与非耦合表象变换
$
  Psi_(l,j=l+1 / 2,m_j)&=1 / sqrt(2j) mat(sqrt(j+m_j)Y_(l,m_j-1 / 2); sqrt(j-m_j)Y_(l,m_j+1 / 2))\ Psi_(l,j=l-1 / 2,m_j)&=1 / sqrt(2j+2) mat(-sqrt(j-m_j+1)Y_(l,m_j-1 / 2); sqrt(j+m_j+1)Y_(l,m_j+1 / 2))
$$
  Y_(l m_l chi_(1 / 2))=sqrt((l+m_l+1) / (2l+1))Psi_(l,j=l+1 / 2,m_l+1 / 2)\ -sqrt((l-m_l) / (2l+1))Psi_(l,j=l-1 / 2,m_l+1 / 2)\ Y_(l m_l chi_(-1 / 2))=sqrt((l-m_l+1) / (2l+1))Psi_(l,j=l+1 / 2,m_l-1 / 2)\ +sqrt((l+m_l) / (2l+1))Psi_(l,j=l-1 / 2,m_l-1 / 2)
$
#def("自旋三重态与单态")[选择${va(S)^2,S_z}$作为对易自旋力学量完全集$ va(S)^2 chi_(S M_S)=&S(S+1)hbar^2chi_(S M_S)\ S_z chi_(S M_S)=&M_S hbar chi_(S M_S) $
  #figure(
    table(
      stroke: none,
      columns: 3,
...
#thm("一级Dirac近似公式")[从$t=0$到$t$时刻,体系从$ket(k)$跃迁到$ket(k')$的概率幅$ C_(k' k) (t)=1 / (i hbar)integral_0^t H'_(k' k) (t) e^(i omega_(k' k)t)dd(t) $$
    P_(k' k) (t)=1 / hbar^2 abs(integral_0^t H'_(k' k) (t) e^(i omega_(k' k)t)dd(t))^2\ H'_(k' k)(t)=braket(k', H'(t), k),omega_(k' k)=(E_k'-E_k) / hbar
  $]
== 光的吸收与辐射
$ H=-va(D)dprod va(E) $
= 矩阵形式
#thm("幺正变换")[对2套各自正交归一的基矢,存在幺正算符$ U=sum_n ket(b_n)bra(a_n) $使得$ ket(b_1)=U ket(a_1),dots,ket(b_n)=U ket(a_n) $]
...
= Born 近似法
#thm("Lippman-Schwinger方程")[$
    psi(va(r))=e^(i va(k)dprod va(r))+(2mu) / hbar^2 integral dd(r', 3) G(va(r),va(r'))V(va(r'))psi(va(r'))\ =psi_i (va(r))+ psi_"sc" (va(r))
  $$ G(va(r),va(r'))=G(va(r)-va(r'))=-e^(i k abs(va(r)-va(r'))) / (4pi abs(va(r)-va(r'))) $$
    psi(va(r))=e^(i va(k)dprod va(r))-mu / (2pi hbar^2) integral dd(r', 3) e^(i k abs(va(r)-va(r'))) / abs(va(r)-va(r')) V(va(r)') psi(va(r'))
  $]
#def("散射边界条件")[$ psi_"sc" (va(r)) -->^(r-> infinity) f(theta,phi.alt) e^(i k r) / r $]
#thm("Born近似方法")[将$psi(va(r'))$用零级近似解$e^(i va(k)dprod va(r'))$代替$ psi(va(r))=e^(i va(k)dprod va(r))-mu / (2pi hbar^2) integral dd(r', 3) e^(i k abs(va(r)-va(r'))) / abs(va(r)-va(r')) V(va(r)') e^(i va(k)dprod va(r')) $$
    f(theta,phi.alt)=-mu / (2pi hbar^2) integral dd(r', 3) e^(-i va(q)dprod va(r')) V(va(r'))
  $如$V(va(r))$只与$r$有关,积掉$theta,phi.alt$有$ f(theta)=-(2mu) / (hbar^2q) integral_0^(+infinity) dd(r') r' sin(q r') V(r')\ q=2k sin(theta\/2) $]
#def("散射截面")[$sigma(theta, phi.alt)=1 / j_1 dv(n, Omega)$$ sigma(theta)=abs(f(theta))^2 $]
